...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [payment] = await getDatabase().select().from(paymentSubmissions).where(and(eq(paymentSubmissions.id, id), eq(paymentSubmissions.userId, session.user.id))).limit(1);
    ...
    const [updated] = await getDatabase().update(paymentSubmissions).set({ receiptBlobKey: uploadedKey, receiptSha256: sha256, submittedAt: new Date() }).where(and(eq(paymentSubmissions.id, id), eq(paymentSubmissions.userId, session.user.id), eq(paymentSubmissions.status, "pending"), isNull(paymentSubmissions.receiptBlobKey))).returning({ id: paymentSubmissions.id, status: paymentSubmissions.status, submittedAt: paymentSubmissions.submittedAt });
