...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await inNeonTransaction(async (client) => {
      ...
      const nestedGates = model.backtest_metrics_json.gates as { activationReady?: boolean } | undefined;
      if (model.backtest_metrics_json.activationReady !== true && nestedGates?.activationReady !== true) {
        throw new Error("BACKTEST_GATE_FAILED");
      }
