...
type Settings = { priceEgp: string; methods: Array<{ id: string; label: string; recipient: string; deepLink?: string }>; instructions: string };
...
export function AccountExperience() {
  ...
  async function submitPayment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/payments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ predictionId: data.get("predictionId"), method: data.get("method"), senderIdentifier: data.get("senderIdentifier"), transactionReference: data.get("transactionReference") || undefined, idempotencyKey: crypto.randomUUID() }) });
      const result = await response.json(); if (!response.ok) throw new Error(result.error);
      if (!(receipt instanceof File) || !receipt.size) throw new Error("اختر صورة الإيصال.");
  ...
  return <div className="grid gap-6 lg:grid-cols-[.75fr_1.25fr]">
    ...
    <div className="grid content-start gap-6">
      ...
      {selectedPrediction && settings ? <form onSubmit={submitPayment} className="border-2 border-teal-700 bg-white p-5"><h2 className="text-xl font-extrabold text-[#173a55]">افتح تقريرك الكامل — {settings.priceEgp} ج.م</h2><p className="mt-1 text-sm text-slate-500">كل الترشيحات المناسبة ليك، وتحديثات المرحلة الثانية والثالثة لنفس النتيجة. {settings.instructions}</p><input type="hidden" name="predictionId" value={selectedPrediction} /><label className="mt-4 grid gap-1 text-sm font-bold">طريقة الدفع<select name="method" required className="min-h-11 border border-slate-300 px-3">{settings.methods.map((method) => <option key={method.id} value={method.id}>{method.label} — {method.recipient}</option>)}</select></label><label className="mt-3 grid gap-1 text-sm font-bold">رقم/حساب المُرسِل<input name="senderIdentifier" required className="min-h-11 border border-slate-300 px-3 ltr-number" /></label><label className="mt-3 grid gap-1 text-sm font-bold">مرجع العملية (اختياري)<input name="transactionReference" className="min-h-11 border border-slate-300 px-3" /></label><label className="mt-3 grid gap-1 text-sm font-bold">صورة الإيصال — JPEG/PNG/WebP حتى 5MB<input name="receipt" type="file" accept="image/jpeg,image/png,image/webp" required className="border border-slate-300 p-3" /></label><button disabled={loading} className="mt-4 min-h-11 bg-teal-700 px-5 font-bold text-white">إرسال الإيصال وطلب التفعيل</button></form> : null}
